...
const removeErrorById = (errorsObj, errorId) => {
  ...
  for (const errorType in errorsObj) {
    ...
    updated[errorType] = Array.isArray(arr) ? arr.filter((item) => item.error_id !== errorId) : arr;
  }
  return updated;
};
...
export const changeHandler = (modelErrorResponse, modelKey, errorId, isAcceptSelected = false) => ({
  ...modelErrorResponse,

...
export default function SingleErrorV2({ error, onClose = null }) {
  ...
  const handleChange = (isAcceptSelected = false) => {
    onClose?.();
    setErrorResponse(changeHandler(errorResponse, selectedSubTab, error.error_id, isAcceptSelected));
  };
  ...
  return (
    <Paper className="p-2 d-flex flex-column gap-1">
      ...
      <span className="fix-pair d-flex gap-1">
        <IconButton className="corrector-error-icon-button" color="success" onClick={() => handleChange(true)}>
          <CheckCircleIcon fontSize="medium" />
        </IconButton>
        <IconButton className="corrector-error-icon-button" color="error" onClick={() => handleChange(false)}>
